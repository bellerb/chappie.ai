...
# Import our chess class
from game_interface import chess

# Import chappie framework
from chappie.ai.bot import Agent
from chappie.ai.model import Representation, Backbone
from chappie.tools.toolbox import ToolBox

if __name__ == '__main__':
    '''
    game_log = pd.read_csv(f'data/models/test_v7/logs/game_log.csv')
...
'''
        ...
        i = 0
        model_list = []
        for m in os.listdir(f'data/models'):
...
'''
            ...
            player = f'data/models/{model_list[m_choice].split("(")[0].strip()}'
            if p_col == 0:
            ...
            if os.path.exists(f'{player}/logs/game_log(human).csv'):
                game_num = len(pd.read_csv(
                    f'{player}/logs/game_log(human).csv'))
            else:
                game_num = 0
            chess = chess()
            print(
...
'''
                ...
                i = 0
                model_list = []
                for m in os.listdir('data/models'):
                    if m != '.DS_Store' and '(temp)' not in m and os.path.exists(f'data/models/{m}/logs/game_log.csv'):
                        model_list.append(f'{m} ({i})')
                        i += 1
                m_choice = tools.give_options(model_list)
                print()
                ...
                with tqdm(total=10, desc=f'Loading model {player}') as pbar:
                    ...
                    pbar.refresh()
                    agent = Agent(
                        param_name=f'data/models/{player}/parameters.json',
                        train=False
                    )
                    pbar.update(5)
                print(
...
'''
                ...
                with tqdm(total=len(layer_list) + 2, desc=f'Loading {layer_name} layer') as pbar:
                    ...
                    for l in layer_list:
                        ...
                        shortform_layer = shortform_layer.replace(
                            ')', '').strip()
                    ...
                    with open(f'data/models/{player}/logs/game_log.csv') as f:
...
'''
                ...
                if (s_choice == 0):
                    data = pd.read_csv(
                        f'data/models/{player}/logs/game_log.csv')
                elif (s_choice == 1):
...
'''
                ...
                del data
                if os.path.exists(f'data/models/{player}/weights') is False:
                    # Create folder
                    os.makedirs(f'data/models/{player}/weights')
                torch.save({
                    'state_dict': agent.m_weights[layer_name]['model'].state_dict(),
                }, f"data/models/{player}/weights/{agent.m_weights[layer_name]['param']}")
                if os.path.exists(f'{player}/logs/training_log.csv'):
                    t_log = pd.read_csv(f'{player}/logs/training_log.csv')
                else:
                    t_log = pd.DataFrame()
                t_log = pd.concat(
                    [t_log, pd.DataFrame(log)], ignore_index=True)
                t_log.to_csv(
                    f'data/models/{player}/logs/training_log.csv', index=False)
            elif t_choice == 1:
                print(
                    '''
-------------------------------------------------
...
'''
            ...
            e_type = tools.give_options(
                ['Single Game       (s)', 'Tourmanent Style  (t)'])
            if e_type == 0:
                players = []
                print(
                    '''
...
'''
                ...
                m_choice = tools.give_options(model_list)
                players.append(
                    f'data/models/{model_list[m_choice].split("(")[0].strip()}/parameters.json')
                print(
                    '''
-------------------------------------------------
                ...
                for i in range(len(model_list) - 1):
                    for j in range(i + 1, len(model_list)):
                        for x in range(10):
                            ...
                            if x % 2 == 0:
                                players = [
                                    f'data/models/{model_list[i].split("(")[0].strip()}/parameters.json',
                                    f'data/models/{model_list[j].split("(")[0].strip()}/parameters.json'
                                ]
                            else:
                                players = [
                                    f'data/models/{model_list[j].split("(")[0].strip()}/parameters.json',
